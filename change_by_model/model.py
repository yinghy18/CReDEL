...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config, add_pooling_layer=False).to(device)
    ...
    def forward(
        ...
        if evaluation:
            ...
            if labels3 == None:
                return (labelscorefct.loss(logits,labels1.to(device)),labelscorefct.loss(logits,labels2.to(device)))
            else:
                return (labelscorefct.loss(logits,labels1.to(device)),labelscorefct.loss(logits,labels2.to(device)),labelscorefct.loss(logits,labels3.to(device)))
